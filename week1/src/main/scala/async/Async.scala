...
object Async {
  ...
  /**
    ...
    */
  def transformSuccess(eventuallyX: Future[Int]): Future[Boolean] = eventuallyX.map { x => x % 2 == 0 }
  ...
  def recoverFailure(eventuallyX: Future[Int]): Future[Int] = {
    eventuallyX.recoverWith { case _ => Future(-1)}
  }
  ...
  ): Future[(A, B)] = makeAsyncComputation1() flatMap ( comp1 => {
  ...
  def futurize(callbackBasedApi: CallbackBasedApi): FutureBasedApi = {
    class FutureAPIImpl extends FutureBasedApi {
      def computeIntAsync(): Future[Int] = {
        val p = Promise[Int]()
